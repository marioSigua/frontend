<template lang="html">
  <section id="home">
    ...
    <div id="cards">
      ...
      <card title="Sample" code="subj.subject_code" time="subj.subject_desc" ></card>
      <card title="Sample" code="subj.subject_code" time="subj.subject_desc" ></card>
    </div>
  </section>
</template>
...
export default {
  ...
  computed: {
    ...
    autoComplete() {
      ...
      return !this.searchList ? [] : listSubjects.filter((item) => {
          return this.searchList.toLowerCase().split(' ').every((v) => item.subject_name.toLowerCase().includes(v))
      })
    },
    foundData() { return this.autoComplete.find((o) => o.subject_name === this.searchList) },
  },
  ...
  methods: {
    sendDispatch() {
      if (this.foundData){ this.patchRequest(this.foundData.subject_code) }
      else { this.patchRequest(this.payload.subject_code) }
    },
  ...
  mounted() {
    this.$store.dispatch('profSubjects')
    setTimeout(() => { this.$store.dispatch('getSubjects') }, 2000)
  },
