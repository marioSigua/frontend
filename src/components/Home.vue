    ...
    <div class="margin">
      ...
      <b-collapse id="collapse-2" class="colp">
        <b-card>
          <p>
            Subject name:
            <input type="text" list="subjects" v-model="searchList" />

            <datalist id="subjects">
              <option v-for="subj in autoComplete" :key="subj.subject_code">{{
                subj.subject_name
              }}</option>
            </datalist>
            ...
            <input
              v-else
              :value="foundData ? foundData.subject_code : ''"
              type="text"
            />
          </p>
          <span>
            ...
            >
              <b-form-select-option value="">{{
                foundData ? foundData.subject_course : "Select Course"
              }}</b-form-select-option>
              <!-- This slot appears above the options from 'options' prop -->
              <!-- These options will appear after the ones from 'options' prop -->
              <b-form-select-option
                v-for="(subj, index) in options.course"
                :key="index"
                :value="subj"
            ...
            <input
              v-else
              :value="foundData ? foundData.subject_desc : ''"
              type="text"
            />
          </span>
          <p>
            ...
            >
              <b-form-select-option value="">{{
                foundData ? foundData.subject_sem : "Select Sem"
              }}</b-form-select-option>
              <!-- This slot appears above the options from 'options' prop -->
              <!-- These options will appear after the ones from 'options' prop -->
              <b-form-select-option
                v-for="(sem, index) in options.semester"
                :key="index"
                :value="sem"
              >
                {{ sem }}
            ...
            <span>
              ...
              >
                <b-form-select-option value="">{{
                  foundData ? foundData.subject_year : "Select Year"
                }}</b-form-select-option>
                <!-- This slot appears above the options from 'options' prop -->
                <!-- These options will appear after the ones from 'options' prop -->
                <b-form-select-option
                  ...
                  :key="index"
                  :value="foundData ? foundData.subject_year : year"
                >
                  {{ year }}
                </b-form-select-option>
              </b-form-select>
            </span>
            <button class="addbtn" @click="sendDispatch()">
              Add
            </button>
          </p>
        </b-card>
